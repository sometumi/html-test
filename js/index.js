var Fireworks = function(){
	...
	window.requestAnimFrame=function(){
		return window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.oRequestAnimationFrame||window.msRequestAnimationFrame||function(a){
			window.setTimeout(a,60)}}();
	self.rand = function(rMi, rMa){return ~~((Math.random()*(rMa-rMi+1))+rMi);}
	self.init = function(){	
    self.dt = 0;
		...
		self.canvas.width = self.cw = $("#canvas-container").width();
		self.canvas.height = self.ch = $("#canvas-container").height();	
	...
	self.bindEvents = function(){
		...
		if("ontouchstart" in window){
			self.canvas.addEventListener('touchstart', function(e){
				...
				self.mx = e.touches[0].pageX - self.canvasContainer.offset().left;
				self.my = e.touches[0].pageY - self.canvasContainer.offset().top;
...
for(var i = 0;i < 10;i++){
	fworks.handPoint(parseInt(fworks.rand(20,250)),parseInt(fworks.rand(40,80)));
}
